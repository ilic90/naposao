...
@section('content')
	<h1 class="page_title main_page_title">{{ __('translate.admin_reports-ads_h1') }}</h1>
		...
		<ul class="list_of_reports">
			@foreach($reports as $report)
				<li class="list_of_reports_item">
					...
					<!--	<span class="list_of_reports_name">{{ $report->user->first_name }}</span> -->
						<span class="list_of_reports_date">{{ $report->created_at->toFormattedDateString() }}</span>
						<span class="list_of_reports_name"><a href="{{ route('getSpecificJob', ['jid' => $report->ad->id]) }}">{{ __('translate.admin_reports-ads_details') }}</a></span>
					</a>
				</li>
			@endforeach
		</ul>
