...
<main class="main_app_container">
	<form action="{{ route('postCompanyEditAd') }}" method="POST" class="new_job_add_choose_form">
		...
		<div class="new_job_add_choose_form_item cf">
			...
			<select class="selectSector select_move_area" size="5">
				@foreach( App\Category::getCategories() as $category)
			     	<option selected="selected" class="selectSectorItem" value="{{ $category->id }}">{{ $category->name }}</option>
			    @endforeach
			</select>
			...
			<select class="selectSectorSelected select_move_area" multiple name="categories[]" size="5">
		     	@foreach($ad->categories as $category)
					<option class="selectSectorSelectedItem" selected="selected" value="{{ $category->id }}">{{ $category->name }}</option>
				@endforeach
			</select>
		...
		<div class="new_job_add_choose_form_item">
			...
			<label for="external_url" class="full_width">
				<input class="full_width" name="external_url" type="text" value="{{ $ad->external_url }}"></label>
				<input name="ad_type" type="hidden" value="1">

			{{ csrf_field() }}
			<div class="new_job_add_choose_form_submit_btn">
				<input type="submit" class="green_btn" value="{{ __('translate.companies_create_ad_publish_button') }}">
			</div>
		</div>	
	</form>
</main>
